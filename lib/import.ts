...
export function matchBooksToCreators(books: GoodreadsBook[]): MatchedCreator[] {
  ...
  for (const book of books) {
    ...
    for (const creator of creators) {
      ...
      if (
        creatorNameLower.includes(authorLower) ||
        authorLower.includes(creatorNameLower)
      ) {
        bestMatch = { creator, confidence: "high" };
        break;
      }
      ...
      if (
        authorLastName === creatorLastName &&
        authorLastName &&
        authorLastName.length > 2
      ) {
  ...
  for (let batchIndex = 0; batchIndex < batches; batchIndex++) {
    ...
    for (const book of batch) {
      try {
        ...
        progress.errors++;
        errors.push(
          `Failed to import "${book.title}": ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      }
    }
  }
...
export function getImportStats(books: GoodreadsBook[]) {
  ...
  for (const book of books) {
    // Count by shelf
    shelfCounts[book.exclusiveShelf] =
      (shelfCounts[book.exclusiveShelf] || 0) + 1;
  ...
  return {
    ...
    yearCounts,
    averageRating:
      books.reduce((sum, b) => sum + (b.averageRating || 0), 0) /
        books.length || 0,
  };
}

// Validate CSV format
export function validateGoodreadsCSV(csvText: string): {
  valid: boolean;
  error?: string;
} {
  if (!csvText.trim()) {
