...
export interface Activity {
  id: string;
  userId: string;
  type: 'followed_user' | 'followed_creator' | 'liked_work' | 'read_work' | 'saved_creator' | 'shared_lineage';
  targetId: string;
...
export const currentUser: UserProfile = {
  ...
  readWorks: ['sun-also-rises', 'old-man-sea', 'what-we-talk-about', 'left-hand'],
...
export const mockUsers: UserProfile[] = [
  ...
  },
  {
    ...
    readWorks: ['sun-also-rises', 'old-man-sea', 'what-we-talk-about', 'cathedral'],
  ...
  },
  {
    ...
    readWorks: ['sound-fury', 'absalom-absalom', 'blood-meridian', 'the-road', 'sun-also-rises'],
...
export function getFollowers(userId: string): UserProfile[] {
  const user = getUserById(userId);
  if (!user) return [];
  return user.followers.map(id => getUserById(id)).filter((u): u is UserProfile => u !== undefined);
}

export function getFollowing(userId: string): UserProfile[] {
  const user = getUserById(userId);
  if (!user) return [];
  return user.following.map(id => getUserById(id)).filter((u): u is UserProfile => u !== undefined);
}
...
export function getActivityIcon(type: Activity['type']): string {
  switch (type) {
    ...
    case 'followed_creator': return '✨';
    case 'liked_work': return '❤️';
    case 'read_work': return '📖';
    case 'saved_creator': return '🔖';
    case 'shared_lineage': return '🔗';
    default: return '•';
  }
}
