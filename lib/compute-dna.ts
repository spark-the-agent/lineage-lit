...
function computeLiteraryTags(creators: Creator[]): string[] {
  ...
  for (const c of creators) {
    ...
    if (
      bio.includes("minimalis") ||
      bio.includes("spare") ||
      bio.includes("iceberg")
    ) {
      tags["Minimalist"] = (tags["Minimalist"] || 0) + 2;
    }
    if (
      bio.includes("science fiction") ||
      bio.includes("speculative") ||
      bio.includes("dystop")
    ) {
      tags["Speculative"] = (tags["Speculative"] || 0) + 2;
    }
    if (
      bio.includes("modernist") ||
      bio.includes("modernism") ||
      bio.includes("stream of consciousness")
    ) {
      tags["Modernist"] = (tags["Modernist"] || 0) + 2;
    }
    if (
      bio.includes("magic realism") ||
      bio.includes("magical realist") ||
      bio.includes("surreal")
    ) {
      tags["Magical Realist"] = (tags["Magical Realist"] || 0) + 2;
    }
    if (
      bio.includes("postmodern") ||
      bio.includes("maximalist") ||
      bio.includes("metafiction")
    ) {
      tags["Postmodernist"] = (tags["Postmodernist"] || 0) + 2;
    }
    if (
      bio.includes("screenplay") ||
      bio.includes("screenwriter") ||
      bio.includes("dialogue")
    ) {
      tags["Cinephile"] = (tags["Cinephile"] || 0) + 2;
    }
    if (
      bio.includes("humanist") ||
      bio.includes("empathy") ||
      bio.includes("compassion")
    ) {
    ...
    if (
      bio.includes("afrofutur") ||
      bio.includes("african") ||
      bio.includes("postcolonial")
    ) {
