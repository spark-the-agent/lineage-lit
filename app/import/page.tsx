...
export default function ImportPage() {
  ...
  const { toggleSavedCreator, isCreatorSaved, recordCreatorView } =
    usePersistence();
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const { books: parsedBooks, errors: parseErrors } =
          parseGoodreadsCSV(text);

        if (parsedBooks.length === 0) {
          setUploadState({
            status: "error",
            message: "No books found in file",
          });
          return;
        }
        ...
        setUploadState({
          status: "complete",
          message: `Found ${parsedBooks.length} books`,
        });
      } catch (err) {
        setUploadState({
          status: "error",
    ...
    [isCreatorSaved, toggleSavedCreator, recordCreatorView],
  );
      ...
      <header className="border-b border-zinc-800/50 hidden lg:block">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 min-h-[44px]">
            ...
            <h1 className="text-xl sm:text-2xl font-bold bg-linear-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">
              Lineage Lit
            </h1>
          </Link>
          <DesktopNav />
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="text-center mb-8 sm:mb-12">
          <h2 className="text-3xl sm:text-4xl font-bold mb-4">
            Import Your <span className="text-amber-400">Goodreads</span>{" "}
            Library
          </h2>
          <p className="text-zinc-400 max-w-xl mx-auto text-base sm:text-lg">
            Import your Goodreads export to discover connections between your
            reading history and our creator database.
          </p>
        </div>
          ...
          <div className="bg-zinc-900/50 rounded-2xl p-6 sm:p-8 border border-zinc-800 mb-8">
            ...
            <ol className="space-y-3 text-zinc-400 text-sm sm:text-base">
              <li className="flex gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center text-sm font-medium">
                  1
                </span>
              ...
              <li className="flex gap-3">
                ...
                <span>
                  Upload the file below — we&apos;ll match books to our creator
                  database
                </span>
              </li>
            </ol>
              ...
              {uploadState.status === "error" ? (
                ...
                <>
                  ...
                  <p className="text-zinc-600 text-sm mt-4">
                    Supports Goodreads export CSV files
                  </p>
                </>
              )}
            </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-zinc-500 text-sm">
                      {stats.totalBooks} books processed
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  ...
                  <div className="text-center p-4 bg-amber-500/10 rounded-xl border border-amber-500/20">
                    <div className="text-2xl font-bold text-amber-400">
                      {mediumConfidenceMatches.length}
                    </div>
                    <div className="text-xs text-zinc-500 mt-1">
                      Medium Confidence
                    </div>
...
function ResultStat({
  ...
  return (
    <div
      className={`p-4 rounded-xl ${highlight ? "bg-amber-500/10 border border-amber-500/20" : "bg-zinc-800/50"}`}
    >
      <div className={`mb-2 ${highlight ? "text-amber-400" : "text-zinc-400"}`}>
        {icon}
      </div>
      <div
        className={`text-2xl font-bold ${highlight ? "text-amber-400" : "text-zinc-100"}`}
      >
        {value}
      </div>
