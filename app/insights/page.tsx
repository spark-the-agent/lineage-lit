import { BarChart3, Network, TrendingUp, GitBranch, Link as LinkIcon, Layers } from 'lucide-react';
...
export const metadata: Metadata = {
  ...
  description: 'Deep network intelligence: influence rankings, literary clusters, bridge creators, and graph analysis.',
};

export default function InsightsPage() {
  const metrics = analyzeNetwork(creators);
  const clusters = getMovementClusters(creators);
        ...
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-8 mb-8">
          ...
          <div className="bg-zinc-900/50 rounded-2xl p-6 border border-zinc-800">
            ...
            <p className="text-zinc-400 text-sm mb-4">
              Creators with high betweenness centrality — they connect otherwise separate communities.
            </p>
            {metrics.bridges.length > 0 ? (
              <div className="space-y-3">
                {metrics.bridges.map((bridge, i) => (
                  <Link
                  ...
                  >
                    <div>
                      <span className="text-sm font-medium text-zinc-200">{bridge.creator.name}</span>
                      <p className="text-xs text-zinc-500">{bridge.creator.years}</p>
                    </div>
              ...
              <p className="text-zinc-500 text-sm">No bridge creators detected in the current network.</p>
            )}
          </div>
        </div>
            ...
            <div className="space-y-3">
              {metrics.pathLengths.slice(0, 5).map((pl, i) => (
                <div key={i} className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs text-zinc-600 ml-2">({pl.length} steps)</span>
                </div>
              ))}
            </div>
          ) : (
