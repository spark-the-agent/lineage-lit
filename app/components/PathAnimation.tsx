    ...
    <div className="space-y-4">
          ...
          >
            ...
            >
              <div className="bg-zinc-900 rounded-xl px-4 py-3 border-2 border-amber-500/50 group-hover:border-amber-400 transition shadow-lg shadow-amber-500/10">
                ...
                <div className="text-[10px] sm:text-xs text-zinc-500">{creator.years}</div>
              </div>
              {/* Glow effect */}
              <div
            ...
            {i < path.length - 1 && (
              <div
                ...
                style={{ animation: `fadeInUp 0.5s ease-out ${i * 0.3 + 0.15}s both` }}
              >
      ...
      <div className="text-center">
        <p className="text-lg font-semibold text-zinc-200">
          <span className="text-amber-400">{path[0].name}</span>
          {' '}to{' '}
          <span className="text-amber-400">{path[path.length - 1].name}</span>
          {' '}in{' '}
          <span className="text-amber-400">{path.length - 1} step{path.length - 1 !== 1 ? 's' : ''}</span>
        </p>
      </div>
