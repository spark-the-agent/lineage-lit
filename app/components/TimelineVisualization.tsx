...
export default function TimelineVisualization() {
  ...
  const sortedCreators = useMemo(() =>
    [...creators].sort((a, b) => parseYears(a.years).start - parseYears(b.years).start),
  []);
      ...
      <div className="min-w-[800px]">
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {Object.entries(clusterColors).map(([era, color]) => (
            <div key={era} className="flex items-center gap-1.5 text-xs text-zinc-400">
        ...
        {/* Timeline */}
        <svg viewBox={`0 0 900 ${sortedCreators.length * 70 + 80}`} className="w-full">
          {/* Decade markers */}
          {decades.map(year => {
            ...
            return (
              <g key={year}>
                <line
                  x1={x} y1={30} x2={x} y2={sortedCreators.length * 70 + 50}
                  stroke="#3f3f46" strokeWidth="1" strokeDasharray="4,4"
                />
                <text x={x} y={20} textAnchor="middle" fill="#71717a" fontSize="11">
                  {year}
                </text>
              </g>
            );
          ...
          {sortedCreators.map((creator, i) => {
            ...
            const era = creatorClusterMap.get(creator.id) || 'Mid-Century';
            ...
            return (
              <g key={creator.id}>
                {/* Creator bar */}
                <rect
                  x={x1} y={y} width={Math.max(x2 - x1, 4)} height={30}
                  rx={4} fill={color} opacity={0.3}
                />
                <rect
                  x={x1} y={y} width={Math.max(x2 - x1, 4)} height={30}
                  rx={4} fill="none" stroke={color} strokeWidth={1.5}
                />

                {/* Creator name */}
                <text x={x1 - 5} y={y + 20} textAnchor="end" fill="#e4e4e7" fontSize="12" fontWeight="500">
                  {creator.name.split(' ').pop()}
                </text>

                {/* Works as dots */}
                {creator.works.map(work => {
                  const wx = ((work.year - TIMELINE_START) / TIMELINE_RANGE) * 800 + 50;
                  return (
                    <g key={work.id}>
                      <circle cx={wx} cy={y + 15} r={4} fill={color} />
                      <title>{work.title} ({work.year})</title>
                    </g>
                  );
                })}
              </g>
          ...
          {creators.map(creator => {
            const fromIdx = sortedCreators.findIndex(c => c.id === creator.id);
            const fromY = fromIdx * 70 + 65;
            const { end: fromEnd } = parseYears(creator.years);
            const fromX = ((fromEnd - TIMELINE_START) / TIMELINE_RANGE) * 800 + 50;

            return creator.influenced.map(targetId => {
              const toIdx = sortedCreators.findIndex(c => c.id === targetId);
              ...
              const toX = ((toStart - TIMELINE_START) / TIMELINE_RANGE) * 800 + 50;

              return (
                <line
                  key={`${creator.id}-${targetId}`}
                  x1={fromX} y1={fromY}
                  x2={toX} y2={toY}
